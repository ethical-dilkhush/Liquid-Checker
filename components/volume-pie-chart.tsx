...
export function VolumePieChart() {
    ...
    async function fetchStats() {
      try {
        const res = await fetch(
          "https://dev.liquidlaunch.app/api/tokens?page=1&limit=4000&search=&sortKey=volume&sortOrder=desc&timeframe=24h&view=all&marketCapMin=0&marketCapMax=1000000&progressMin=0&progressMax=100&filterByHolderCount=false",
        )
        ...
        tokens.forEach((token: any) => {
          ...
          if (token.progress >= 50 && token.progress < 100) {
            closeToBond++
          }
        })
        ...
        setStats((s) => ({ ...s, loading: false, error: "Failed to fetch volume stats" }))
      }
    }
    fetchStats()
  }, [])
  ...
  if (stats.loading) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Volume Stats</CardTitle>
          <CardDescription className="text-sm sm:text-base">
            The data displayed in the Stats section of the volume is now also visualized in a pie chart format for
  ...
  const data = [
    ...
    {
      name: "Close to Bond",
      value: stats.closeToBond * 100000,
      actualValue: stats.closeToBond,
      formattedValue: stats.closeToBond.toString(),
    },
  ]
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-lg sm:text-xl">Volume Stats</CardTitle>
        <CardDescription className="text-sm sm:text-base">
          The data displayed in the Stats section of the volume is now also visualized in a pie chart format for better
