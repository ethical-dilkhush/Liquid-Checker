...
const COLORS = ["#60A5FA", "#F472B6", "#A3E635", "#C084FC"]

export function BondedPieChart() {
    ...
    async function fetchStats() {
      try {
        const res = await fetch(
          "https://dev.liquidlaunch.app/api/tokens?page=1&limit=4000&search=&sortKey=age&sortOrder=desc&timeframe=24h&view=bonded&marketCapMin=0&marketCapMax=1000000&progressMin=0&progressMax=100&filterByHolderCount=false",
        )
        ...
        setStats({
          totalBondedTokens: tokens.length,
          totalMarketCap,
          totalLiquidity,
          totalHolders,
          loading: false,
        ...
        console.error("Error fetching bonded stats:", e)
        setStats((s) => ({ ...s, loading: false, error: "Failed to fetch bonded stats" }))
      }
    }
    fetchStats()
  }, [])
  ...
  if (stats.loading) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl">Bonded Stats</CardTitle>
          <CardDescription className="text-sm sm:text-base">
            The data displayed in the Stats section of the bonded is now also visualized in a pie chart format for
  ...
  const data = [
    {
      ...
      value: stats.totalBondedTokens * 10000,
      actualValue: stats.totalBondedTokens,
      formattedValue: stats.totalBondedTokens.toLocaleString(),
    },
    {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-lg sm:text-xl">Bonded Stats</CardTitle>
        <CardDescription className="text-sm sm:text-base">
          The data displayed in the Stats section of the bonded is now also visualized in a pie chart format for better
