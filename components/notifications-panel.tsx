...
export function NotificationsPanel() {
    ...
    async function fetchToken() {
      try {
        ...
        const highest = tokens.reduce(
          (max: any, t: any) =>
            Number(t.timeframes?.["24h"]?.priceChange || 0) > Number(max.timeframes?.["24h"]?.priceChange || 0)
              ? t
              : max,
          tokens[0],
        )
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Highest Price Change</CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Price Change (24 Hour)</CardTitle>
      </CardHeader>
      <CardContent>
