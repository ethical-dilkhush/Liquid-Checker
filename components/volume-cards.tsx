...
export function VolumeCards({ tokens, loading, error }: VolumeCardsProps) {
  const { toast } = useToast()
  const [currentPage, setCurrentPage] = useState(1)
  const [copied, setCopied] = useState<string | null>(null)
  const tokensPerPage = 20

  // Format currency with K, M, B suffixes - completely safe version
  ...
  const formatTimeAgo = (timestamp: number): string => {
    ...
    try {
      const now = new Date()
      const createdDate = new Date(timestamp * 1000) // Convert to milliseconds
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-text-muted truncate">{token.name}</p>
                </div>
                ...
                <div className="bg-gray-800/50 rounded-lg p-3 text-center">
                  ...
                  <div
                    className={`font-semibold text-sm ${
                      token.timeframes?.["24h"]?.priceChange &&
                      Number.parseFloat(token.timeframes["24h"].priceChange) > 0
                        ? "text-green-500"
                        : token.timeframes?.["24h"]?.priceChange &&
                            Number.parseFloat(token.timeframes["24h"].priceChange) < 0
                          ? "text-red-500"
                  ...
                  >
                    {token.timeframes?.["24h"]?.priceChange
                      ? `${Number.parseFloat(token.timeframes["24h"].priceChange) > 0 ? "+" : ""}${Number.parseFloat(token.timeframes["24h"].priceChange).toFixed(2)}%`
                      : "N/A"}
                  </div>
                </div>
              </div>
